...
public class CheckForNearbyAnts : MonoBehaviour
{
    ...
    private EnemyAntMovement enemyAnt;
    void Start()
    {
        background = GameObject.Find("Background");
        backgroundGrid = background.GetComponent<BackgroundGrid>();
        enemyAnt = GetComponent<EnemyAntMovement>();
    }
    ...
    public void CheckSurroundingArea()
    {
        ...
        foreach (Collider2D ant in surroundingAnts)
        {
            ...
            if (enemyAnt.GetPreyedAnt() == ant.gameObject && ant.gameObject && ant.gameObject.GetComponent<Ant>().GetDead()) enemyAnt.SetPreyedAnt(null);
            if (ant.gameObject.CompareTag("Ant") && !ant.gameObject.GetComponent<Ant>().GetDead())
            {
