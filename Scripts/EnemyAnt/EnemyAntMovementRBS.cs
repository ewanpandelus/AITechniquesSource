...
public class EnemyAntMovementRBS : MonoBehaviour
{
    ...
    private CheckForNearbyAntsRBS checkForNearbyAntsRBS;
    ...
    private void Start()
    {
        ...
        checkForNearbyAntsRBS = GetComponent<CheckForNearbyAntsRBS>();
    ...
    private void FixedUpdate()
    {
        ...
        {
            checkForNearbyAntsRBS.CheckSurroundingArea();
            preyedAnt = checkForNearbyAntsRBS.AssessAntPosition();
            Movement(true);
        }
    }
    ...
    {
        if (preyedAnt != null && !preyedAnt.GetComponent<RBSAnt>().GetDead())
        {
    ...
    private void Decelerate()
    {
        if (shouldTurnAround)
        {
            if (maxVariedSpeed > 0.5f) maxVariedSpeed -= maxVariedSpeed / 2;
            if (minVariedSpeed > 0.3f)  minVariedSpeed -= minVariedSpeed / 100;
        }
    }
